...
VarSlot* pool_alloc() {
    for(int i=0;i<fixed_top;i++) if(!fixed_pool[i].in_use){ fixed_pool[i].in_use=1; return &fixed_pool[i];}
    if(fixed_top<FIXED_VARS){ VarSlot* slot=&fixed_pool[fixed_top++]; slot->in_use=1; slot->last_use=-1; slot->constant=0; return slot; }
    VarPoolChunk* chunk=dynamic_pool;
    while(chunk){ for(int i=0;i<chunk->capacity;i++) if(!chunk->slots[i].in_use){ chunk->slots[i].in_use=1; chunk->slots[i].last_use=-1; chunk->slots[i].constant=0; return &chunk->slots[i]; } chunk=chunk->next;}
...
void free_vars_after(int ir_index) {
    for(int i=0;i<fixed_top;i++) if(fixed_pool[i].in_use && fixed_pool[i].last_use<=ir_index){ fixed_pool[i].in_use=0;}
    VarPoolChunk* chunk=dynamic_pool;
    while(chunk){ for(int i=0;i<chunk->capacity;i++) if(chunk->slots[i].in_use && chunk->slots[i].last_use<=ir_index){ chunk->slots[i].in_use=0;} chunk=chunk->next; }
}
...
void build_dependencies(IR* ir){ for(int i=0;i<ir->count;i++){ IRStmt* s=&ir->stmts[i]; s->dep_count=0; s->dep_indices=NULL; s->executed=0; for(int j=0;j<i;j++){ IRStmt* prev=&ir->stmts[j]; for(int k=0;k<s->argc;k++){ if(prev->lhs_index==s->arg_indices[k]){ s->dep_count++; s->dep_indices=realloc(s->dep_indices,sizeof(int)*s->dep_count); s->dep_indices[s->dep_count-1]=j; break; } } } } }

void init_env(int total_vars){ env_array=malloc(sizeof(VarSlot*)*total_vars); for(int i=0;i<total_vars;i++){ env_array[i]=malloc(sizeof(VarSlot)); env_array[i]->data=NULL; env_array[i]->constant=0; env_array[i]->value=0; } var_count=0; }
void free_env(){ for(int i=0;i<var_count;i++){ if(env_array[i]->data) free(env_array[i]->data); free(env_array[i]); } free(env_array); }
...
int main(int argc,char** argv){
    ...
    // Call .bin executor
    if(func_count>0 && func_table[0].ptr) { void (*bin_exec)(IR*, VarSlot**) = func_table[0].ptr; bin_exec(&ir, env_array); }
