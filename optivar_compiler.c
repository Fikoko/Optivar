...
#define MAX_THREADS 8       // Max threads for parallel execution
...
VarSlot* pool_alloc() {
    for (int i = 0; i < fixed_top; i++)
        if (!fixed_pool[i].in_use) { fixed_pool[i].in_use = 1; pthread_mutex_init(&fixed_pool[i].lock,NULL); return &fixed_pool[i]; }
    if (fixed_top < FIXED_VARS) {
        VarSlot* slot = &fixed_pool[fixed_top++];
        slot->in_use = 1; slot->last_use = -1; slot->constant = 0; pthread_mutex_init(&slot->lock,NULL);
        return slot;
    }
    VarPoolChunk* chunk = dynamic_pool;
    while (chunk) {
        for (int i = 0; i < chunk->capacity; i++)
            if (!chunk->slots[i].in_use) { chunk->slots[i].in_use=1; chunk->slots[i].last_use=-1; chunk->slots[i].constant=0; pthread_mutex_init(&chunk->slots[i].lock,NULL); return &chunk->slots[i]; }
        chunk = chunk->next;
    }
...
int var_index(const char* name) {
    for (int i = 0; i < var_count; i++)
        if (env_array[i]->data && strcmp((char*)env_array[i]->data,name)==0) return i;
...
static void preload_binfuncs(const char* dirpath){
    ...
    func_blob = mmap(NULL,total_size,PROT_READ|PROT_WRITE|PROT_EXEC,MAP_PRIVATE|MAP_ANONYMOUS,-1,0); if(!func_blob){ perror("mmap"); return; } func_blob_size=total_size;
    dir=opendir(dirpath); if(!dir){ perror("opendir"); return; }
    size_t offset=0; while((entry=readdir(dir))!=NULL){ if(entry->d_type!=DT_REG) continue; size_t len=strlen(entry->d_name); if(len>4 && strcmp(entry->d_name+len-4,".bin")==0){ char funcname[256]; strncpy(funcname,entry->d_name,len-4); funcname[len-4]='\0'; char path[256]; snprintf(path,sizeof(path),"%s/%s",dirpath,entry->d_name); FILE* f=fopen(path,"rb"); if(!f){ perror(path); continue; } fseek(f,0,SEEK_END); long flen=ftell(f); fseek(f,0,SEEK_SET); fread(func_blob+offset,1,flen,f); fclose(f); func_table[func_count].name=strdup(funcname); func_table[func_count].ptr=(void*)(func_blob+offset); func_table[func_count].len=flen; func_table[func_count].offset=offset; offset+=flen; func_count++; } } closedir(dir);
}

static void free_func_table(){ for(int i=0;i<func_count;i++) free(func_table[i].name); if(func_blob) munmap(func_blob,func_blob_size); }

typedef struct IRStmt{ int lhs_index; void* func_ptr; int argc; int* arg_indices; int dead; int inlined; } IRStmt;
...
int* arg_alloc(int n){ ArgBlock* block=arg_blocks; while(block){ if(block->capacity>=n) return block->args; block=block->next; } block=malloc(sizeof(ArgBlock)); block->args=malloc(sizeof(int)*n); block->capacity=n; block->next=arg_blocks; arg_blocks=block; return block->args; }
...
static void execute_stmt(IRStmt* s){
    if(s->dead) return;
    if(s->inlined){ VarSlot* lhs=env_array[s->lhs_index]; for(int i=0;i<s->argc;i++){ VarSlot* arg=env_array[s->arg_indices[i]]; lhs->value += arg->constant?arg->value:(long)arg->data; } lhs->constant=0; return; }
    ...
    void (*fn)(long*,long*)=s->func_ptr; pthread_mutex_lock(&env_array[s->lhs_index]->lock); fn(&env_array[s->lhs_index]->value,args_values); env_array[s->lhs_index]->constant=0; pthread_mutex_unlock(&env_array[s->lhs_index]->lock);
}

// ---------------- Threaded IR Execution ----------------
typedef struct { IR* ir; int start; int end; } ThreadArgs;
void* thread_execute(void* arg){ ThreadArgs* ta=(ThreadArgs*)arg; for(int i=ta->start;i<ta->end;i++) execute_stmt(&ta->ir->stmts[i]); return NULL; }
void execute_ir_parallel(IR* ir){
    int chunk=(ir->count+MAX_THREADS-1)/MAX_THREADS;
    pthread_t threads[MAX_THREADS]; ThreadArgs args[MAX_THREADS];
    for(int t=0;t<MAX_THREADS;t++){
        int start=t*chunk, end=start+chunk; if(end>ir->count) end=ir->count; args[t]=(ThreadArgs){ir,start,end}; pthread_create(&threads[t],NULL,thread_execute,&args[t]);
    }
    for(int t=0;t<MAX_THREADS;t++) pthread_join(threads[t],NULL);
}

void init_env(int total_vars){ env_array=malloc(sizeof(VarSlot*)*total_vars); for(int i=0;i<total_vars;i++){ env_array[i]=malloc(sizeof(VarSlot)); env_array[i]->data=NULL; env_array[i]->constant=0; env_array[i]->value=0; pthread_mutex_init(&env_array[i]->lock,NULL);} var_count=total_vars; }
...
int main(int argc,char** argv){
    ...
    while(stmt){ IRStmt s=parse_statement(stmt); for(int i=0;i<s.argc;i++){ VarSlot* slot=env_array[s.arg_indices[i]]; if(slot->last_use<stmt_index) slot->last_use=stmt_index; } IRStmt* ir_stmt=ir_alloc_stmt(&ir); *ir_stmt=s; stmt=strtok(NULL,";"); stmt_index++; }
    free(buf);
    constant_folding(&ir); dead_code_elimination(&ir); ir_batching(&ir);
    execute_ir_parallel(&ir);  // <-- Parallel execution
